...
if [ -z "$BASE_DOMAIN" ]; then
  ...
  # Движки и компоненты открываются в iframe со страницы Life OS, поэтому CSP должен
  # разрешать их домены. Здесь задаём всё семейство (<id>.example.com для lifeos.example.com);
  # установщик движка дополнительно вписывает в CSP свой домен, если тот вне семейства.
  case "$OS_DOMAIN" in
    *.*.*) FRAME_WILD="*.${OS_DOMAIN#*.}" ;;
    *)     FRAME_WILD="*.${OS_DOMAIN}" ;;
  esac

  echo "   Генерирую Caddyfile для: $OS_DOMAIN (Life OS)"
  echo "   Домены движков в CSP: https://$FRAME_WILD"
...
# Домены движков (Hermes, OpenCode) добавляются в этот файл при их установке из панели.
