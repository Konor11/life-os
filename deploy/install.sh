...
#   6. (опционально) Caddyfile для базового домена Life OS (os.<domain>)
#      + инфраструктурных поддоменов (admin, hermes, workspace)
#
...
# Их поддомены (oc.*, ds.*, n8n.*) настраиваются при установке компонента.
# =============================================================================
set -euo pipefail
...
if [ -z "$BASE_DOMAIN" ]; then
  ...
  echo "   Поддомены движков (oc.*, ds.*, n8n.*) добавляются при установке компонентов через UI."
...
# Engine subdomains (oc, ds, n8n) are added by component installers.
...
# Main Life OS domain: API, WS, SPA fallback.
$OS_DOMAIN {
    handle /api/* {
        reverse_proxy 127.0.0.1:3004
    }
